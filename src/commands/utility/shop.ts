...
const buildShopLogger = (interaction: any, context = {}, meta = {}) => (
...
const purchaseSessions = new Map<string, any>();

function formatPrice(value: number) {
  return `${value} コイン`;
}

function buildItemEmbed(item: any) {
...
async function handleItemAdd(interaction: any) {
  const guildId = interaction.guildId;
  if (!guildId) {
    await interaction.reply({ content: 'ギルド内でのみ利用できます。', ephemeral: true });
    return;
  }
  // defer the reply to avoid Discord interaction timeout while we perform DB operations
  // Use flags instead of deprecated `ephemeral` option (EPHEMERAL = 1 << 6)
  ...
  try {
    ...
    await interaction.editReply({ content: 'アイテムを追加しました。' }).catch(() => null);
  } catch (error: any) {
    // Log with context
    buildShopLogger(interaction, { event: 'shop.item.add.error', guild_id: guildId, item_name: name }).error({ err: error }, 'Failed to add shop item');

    // Handle known Prisma error codes with friendly messages.
    if (error?.code === 'P1001') {
      // Can't reach DB server
      await interaction.editReply({ content: 'データベースに接続できません。しばらくしてから再試行してください。' }).catch(() => null);
      return;
    }

    if (error?.code === 'P2021') {
      // Table/model missing in DB schema
      await interaction.editReply({ content: 'データベースのスキーマが準備できていません。マイグレーションを実行してください。' }).catch(() => null);
      return;
    }

    // Generic error response
    await interaction.editReply({ content: 'アイテムの追加中にエラーが発生しました。' }).catch(() => null);
  }
}

async function handleItemRemove(interaction: any) {
  const guildId = interaction.guildId;
  if (!guildId) {
    await interaction.reply({ content: 'ギルド内でのみ利用できます。', ephemeral: true });
    return;
...
function buildSelectMenuWithDefault(customId: string, placeholder: string, options: any[]) {
...
async function handleAnnounceSend(interaction: any) {
  const guildId = interaction.guildId;
  if (!guildId) {
    await interaction.reply({ content: 'ギルド内でのみ利用できます。', ephemeral: true });
    return;
...
async function handleAnnounceDelete(interaction: any) {
  const guildId = interaction.guildId;
  if (!guildId) {
    await interaction.reply({ content: 'ギルド内でのみ利用できます。', ephemeral: true });
    return;
  ...
  const sorted = Array.from(candidates.values()).sort((a: any, b: any) => b.createdTimestamp - a.createdTimestamp);
  const limited = sorted.slice(0, 24);

  const options = limited.map((message: any) => {
...
export async function handleShopCancel(interaction: any, sessionId: string) {
...
export async function handleShopConfirm(interaction: any, sessionId: string) {
  const session = getSession(sessionId);
  if (!session) {
    await interaction.reply({ content: 'セッションの有効期限が切れています。', ephemeral: true });
    return;
  ...
  clearSession(sessionId);
  // debit() may return either a number or an object with a nested balance property depending on service implementation.
  // Be defensive here during incremental migration.
  const remaining = (purchase as any)?.balance?.balance ?? (typeof purchase === 'number' ? purchase : undefined);
...
export async function handleShopItemRemoveSelect(interaction: any) {
...
export async function handleShopAnnounceDeleteSelect(interaction: any) {
